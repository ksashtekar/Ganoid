	   ...
	   int fieldwidth)
{
	...
	if (((unsigned)value & 0x80000000UL) &&
	    (base != 16) && (base != 2) && (!unsignedvalue)
	    ) {
	...
	if (fieldwidth > 0) {
		if ((fieldwidth - no_of_digits) < 0) {
			/* digits will not fit in given fieldwidth */
			fillbytes = 0;
		} else
	...
	i--;
	for (j = i, div = 1; j; j--, div *= (long)base)
		;

	for (; div; div /= (long)base, str++) {
		d = (int)(abs_value / (unsigned)div);	 /* get the digit */
		*str = carray[d];
		abs_value = abs_value - (unsigned)(d * div);
	}
...
int vsprintf(char *buf, const char *fmt, va_list vl)
{
	...
	while (1) {
		...
		if (isdigit(*fmt)) {	/* if true fieldwidth is present */
			...
			/* fieldwidth is provided as argument */
		...
		fmt++;
		realize_string(&buf, flags, fieldwidth, precision,
			       length_modifier, conv_specifier, &vl);
		fieldwidth = 0;
		precision = 0;
		length_modifier = 0;
		conv_specifier = 0;
...
void realize_string(char **buf, int flags __attribute__ ((unused)),
		    int fieldwidth, int precision __attribute__ ((unused)),
		    int length_modifier __attribute__ ((unused)),
		    int conv_specifier, va_list *vl)
{
	char *mbuf;
	char *ch;
	char ch1;
...
#undef STR_SIZE
	...
	case 'u':
		...
		break;
	case 'c':
		ch1 = (char)va_arg(*vl, int);
		*mbuf++ = ch1;
		break;
	default:
		*mbuf = 0;
		vga_puts(mbuf);
		vga_putc((char)conv_specifier);
		vga_putc(':');
...
int get_value(const char **fmt)
{
	...
	*fmt = mfmt;
	...
	/* value of 'i' is set in above loop. this value signifies
