...
static int head = 0; // always points to byte to be written
static int tail = 0; // points to the last byte of the circular buffer

static void keyboard_state_machine (unsigned char i, int *o, 
				    bool *make, bool *repeat_key,
				    bool *basic_key);
static void map_scancode_to_key (int scancode, bool *printable);
...
{ 
	...
	*printable = true;
	...
	/*
	// test code
	unsigned char testarr[]={0x1C, 0x1C, 0xF0, 0x1C, 0xE0, 0x1F, 0xE0, 0x1F, 0xE0, 0x00, 0xF0, 0x1F};
	int o, make;
	...
	if (make || repeat_key) {
		...
		if (basic_key) {
			r = binary_search_key(scan_code, &basic_keymap, 
					      sizeof(basic_keymap)/sizeof(struct keymap), 
					      &val);
			*key = val;
			if (0 == r)
				if (val & 0x100) // b'coz we have used such an encoding
					*printable = false;
				else
					*printable = true;
			else
			...
			r = binary_search_key(scan_code, &extended_keymap, 
					      sizeof(extended_keymap)/sizeof(struct keymap), 
					      &val);
			*key = val;
			if (0 == r)
				if (val & 0x200) // b'coz we have used such an encoding
					*printable = false;
				else
					*printable = true;
			else
