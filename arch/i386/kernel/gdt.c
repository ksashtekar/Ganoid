...
void init_GDT(void)
{
	...
	add_gdt_entry(1, 0, 0xFFFFF,
		      ...
		      GBL_DESC_DEFAULT_OPERATION_SIZE_32BIT,
		      GBL_DESC_GRANULARITY_4KB);
	// data segment
	...
	GDTR_val.table_limit =
	    (sizeof(struct segment_descriptor) * DEFAULT_GDT_SIZE);
...
void add_gdt_entry(int gdt_index, unsigned seg_base, unsigned seg_limit,
		   unsigned seg_type, unsigned desc_type,
		   unsigned desc_priv_level, unsigned default_operation_size,
		   unsigned granularity)
{
	GDT[gdt_index].seg_limit_low = (seg_limit & 0xFFFF);
	GDT[gdt_index].base_addr_low = (seg_base & 0xFFFF);
	GDT[gdt_index].base_addr_middle =
	    (unsigned char)((seg_base & 0x00FF0000) >> 16);
	GDT[gdt_index].flags_1 = (unsigned char)((GBL_DESC_SEGMENT_PRESENT)
						 | (desc_priv_level << 5) |
						 (desc_type << 4)
						 | (seg_type));
	GDT[gdt_index].flags_2 =
	    (unsigned char)(granularity | default_operation_size |
			    (u32) ((seg_limit & 0x000F0000) >> 16));
	GDT[gdt_index].base_addr_high =
...
{
	int i;
	struct segment_descriptor *seg_desc =
	    (struct segment_descriptor *)GDTR_value->table_base_addr;
	...
	for (i = 0; i < GDTR_value->table_limit; i += 8, seg_desc += 1) {
		...
		segment_base_address =
		    ((unsigned int)seg_desc->
		     base_addr_high) << 24 | ((unsigned int)seg_desc->
					      base_addr_middle) << 16 |
		    ((unsigned int)seg_desc->base_addr_low);
		segment_limit =
		    ((((unsigned int)(seg_desc->
				      flags_2)) & 0x0F) << 16) | (unsigned int)
		    seg_desc->seg_limit_low;
